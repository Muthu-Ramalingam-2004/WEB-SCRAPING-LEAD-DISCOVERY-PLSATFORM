...
export default function DashboardPage() {
  ...
  const kpis = [
    { label: 'Total Tasks', value: totalTasks || 24, icon: ListChecks, color: 'bg-teal-50 dark:bg-teal-950/60 text-teal-600 dark:text-teal-400', trend: '+3 this week' },
    { label: 'Total Leads', value: totalLeads.toLocaleString(), icon: Users, color: 'bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400', trend: '+240 this week' },
    { label: 'Websites Crawled', value: websitesCrawled.toLocaleString(), icon: Globe, color: 'bg-violet-50 dark:bg-violet-950/60 text-violet-600 dark:text-violet-400', trend: '92% success' },
    { label: 'Verified Leads', value: verifiedLeads.toLocaleString(), icon: ShieldCheck, color: 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400', trend: '70% verified' },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-card overflow-hidden">
        <div className="p-6 md:p-8">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-lg font-extrabold text-slate-900 dark:text-slate-100">Find your next leads</h2>
          </div>
          ...
          <form onSubmit={handleQuickStart} className="flex flex-col md:flex-row gap-3">
            ...
            <button
              type="submit"
              className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-slate-900 dark:bg-slate-100 hover:bg-slate-800 dark:hover:bg-white text-white dark:text-slate-900 font-bold text-sm rounded-xl shadow-sm transition-all duration-200 shrink-0"
            >
              Start Scraping
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {kpis.map((kpi) => {
            ...
            return (
              <div
                key={kpi.label}
                className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-subtle hover:shadow-card transition-shadow duration-200"
              >
                ...
                <p className="text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">{kpi.value}</p>
                <div className="flex items-center gap-1 mt-1.5">
                  <TrendingUp className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                  <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold">{kpi.trend}</span>
                </div>
              </div>
            );
          })}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-extrabold text-slate-900 dark:text-slate-100">Recent Scraping Tasks</h3>
          <Link
            href="/history"
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {recentTasks.map((task) => (
                    <tr key={task.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3.5 text-xs text-slate-500 dark:text-slate-400">{task.createdAt}</td>
                      <td className="px-4 py-3.5 text-right">
                        <Link
                          href={task.status === 'RUNNING' ? `/tasks/${task.id}/progress` : `/leads?taskId=${task.id}`}
                          className="text-xs font-semibold text-teal-600 dark:text-teal-400 hover:text-teal-800 dark:hover:text-teal-300 transition-colors inline-flex items-center gap-1"
                        >
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-extrabold text-slate-900 dark:text-slate-100">Recent Verified Leads</h3>
          <Link
            href="/leads"
            className="text-xs font-semibold text-teal-600 dark:text-teal-400 hover:text-teal-800 dark:hover:text-teal-300 transition-colors flex items-center gap-1"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {recentLeads.map((lead) => (
            <Link
              ...
              className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-subtle hover:shadow-card-hover transition-all duration-200 group"
            >
              <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 bg-teal-50 dark:bg-teal-950/60 text-teal-600 dark:text-teal-400 rounded-xl flex items-center justify-center text-sm font-bold shrink-0">
                  {lead.organizationName.charAt(0)}
                </div>
                <ConfidenceBadge confidence={lead.confidence} />
              </div>
              <h4 className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-0.5 group-hover:text-teal-700 dark:group-hover:text-teal-400 transition-colors line-clamp-1">
                {lead.organizationName}
              </h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">{lead.category} · {lead.city}</p>
              <div className="space-y-1.5">
                {lead.phone && (
                  <p className="text-xs text-slate-600 dark:text-slate-300 truncate">📞 {lead.phone}</p>
                )}
                {lead.email && (
                  <p className="text-xs text-slate-600 dark:text-slate-300 truncate">✉️ {lead.email}</p>
                )}
                {lead.website && (
                  <p className="text-xs text-teal-600 dark:text-teal-400 font-medium truncate">🌐 {lead.website}</p>
                )}
              </div>
            </Link>
          ))}

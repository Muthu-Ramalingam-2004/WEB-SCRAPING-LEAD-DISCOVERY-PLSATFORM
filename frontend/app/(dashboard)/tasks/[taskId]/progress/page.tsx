...
export default function ProgressPage() {
  ...
  const metrics = [
    { label: 'Results Discovered', value: progress.resultsDiscovered, icon: Users, color: 'text-teal-600 bg-teal-50' },
    { label: 'Websites Found', value: progress.websitesFound, icon: Globe, color: 'text-blue-600 bg-blue-50' },
    { label: 'Websites Crawled', value: progress.websitesCrawled, icon: Globe, color: 'text-violet-600 bg-violet-50' },
    { label: 'Phones Found', value: progress.phonesFound, icon: Phone, color: 'text-emerald-600 bg-emerald-50' },
    { label: 'Emails Found', value: progress.emailsFound, icon: Mail, color: 'text-amber-600 bg-amber-50' },
    { label: 'Addresses Found', value: progress.addressesFound, icon: MapPinned, color: 'text-rose-600 bg-rose-50' },
    { label: 'Duplicates Removed', value: progress.duplicatesRemoved, icon: Copy, color: 'text-slate-600 bg-slate-100' },
  ];
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Scraping in progress</h1>
            <StatusBadge status={progress.status} />
          </div>
          <div className="flex items-center gap-4 text-sm text-slate-500">
            <span className="font-mono text-xs font-bold text-slate-600">{taskId}</span>
        ...
        <div className="flex items-center justify-center mb-6">
          <div className="relative w-36 h-36">
            ...
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 120 120">
              <circle cx="60" cy="60" r="52" stroke="currentColor" strokeWidth="10" fill="none" className="text-slate-100" />
              <circle
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-4xl font-extrabold text-slate-900">{animatedPercentage}%</span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {metrics.map((m) => {
          const Icon = m.icon;
          return (
            <div key={m.label} className="bg-white rounded-xl border border-slate-200 shadow-subtle p-4">
              ...
              <p className="text-[11px] text-slate-500 font-semibold uppercase tracking-wider">{m.label}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-card p-6">
          <h3 className="text-sm font-extrabold text-slate-900 mb-5">Activity Timeline</h3>
              ...
              <div key={item.id} className="flex gap-3">
                ...
                <div className="flex flex-col items-center">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${
                    ...
                    item.status === 'in_progress' ? 'bg-teal-100 animate-pulse' :
                    'bg-slate-100'
                  }`}>
                    {item.status === 'completed' && <CheckCircle2 className="w-4 h-4 text-emerald-600" />}
                    {item.status === 'in_progress' && <Loader2 className="w-4 h-4 text-teal-600 animate-spin" />}
        ...
        <div className="space-y-4">
          {/* Current Crawl */}
          <div className="bg-white rounded-2xl border border-slate-200 shadow-card p-6">
            <h3 className="text-sm font-extrabold text-slate-900 mb-4">Current Crawl</h3>
            <div className="space-y-3">
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Current Page</p>
                <p className="text-sm font-mono text-slate-600">{progress.currentPage}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Pages Crawled</p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-card overflow-hidden">
            ...
            >
              ...
              {showFailed ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
            </button>
              ...
              <div className="px-6 pb-4">
                ...
                <div className="space-y-2">
                  {progress.failedWebsites.map((fw) => (
                    <div key={fw.url} className="flex items-center justify-between bg-slate-50 rounded-lg px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <Globe className="w-3.5 h-3.5 text-slate-400" />
                        <span className="text-xs font-medium text-slate-700">{fw.domain}</span>
                      </div>
